...
data=pd.read_csv('LR_HKAG_11_12_eegdata.csv',encoding='utf-8')
...
X_train=X_train.values.reshape(1287,4,1)
X_test=X_test.values.reshape(430,4,1)
...
checkpoint = ModelCheckpoint('LR_HKAG_11_12_eegdata_model-{epoch:03d}-{acc:03f}-{val_acc:03f}.h5', verbose=2, monitor='val_acc',save_best_only=True, mode='auto')
model_classifier = Sequential()
model_classifier.add(LSTM(96, return_sequences = True, input_shape = (X_train.shape[1], 1)))
model_classifier.add(BatchNormalization())

model_classifier.add(LSTM(96, return_sequences = True,activation='relu',bias_regularizer=l2(0.01)))
Dropout(0.2)
model_classifier.add(BatchNormalization())
model_classifier.add(LSTM(96, return_sequences = True,activation='tanh',bias_regularizer=l2(0.01)))
Dropout(0.2)
model_classifier.add(BatchNormalization())
model_classifier.add(LSTM(96,return_sequences = True, activation='relu',bias_regularizer=l2(0.01)))
Dropout(0.2)
...
model_classifier.add(LSTM(96, activation='relu',bias_regularizer=l2(0.001)))
model_classifier.add(BatchNormalization())
...
with open("LR_HKAG_11_12_eegdata.json", "w") as json_file:
